...
from selenium.common.exceptions import NoSuchFrameException, StaleElementReferenceException
import pandas as pd
import re
import time
import datetime
...
# options.add_argument('headless')              # webbrowser 를 안보여줌
...
for l in range(6):
    ...
    for k in range(1, 3):
        ...
        driver.get(url)
        time.sleep(0.5)         # 페이지를 바꾸는 시간을 기다려 줌
        for i in range(1, 5):
            for j in range(1, 6):
...
# xPath 요소 마다 모두 다른 값을 가짐
# //*[@id="section_body"]/ul[1]/li[1]/dl/dt[2]/a            # 인덱스 값은 1부터 시작
